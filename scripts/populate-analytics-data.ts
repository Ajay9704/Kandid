...
async function populateAnalyticsData() {
  ...
  try {
    ...
    if (db) {
      const leads = await db.collection('leads').find({ userId: demoUser.id }).toArray()
      ...
      for (let i = 0; i < Math.min(20, leads.length); i++) {
        ...
        try {
          ...
          console.log(`✅ Created activity log: ${descriptions[activityType as keyof typeof descriptions]}`)
        } catch (error) {
          console.log(`⚠️ Activity log might already exist`)
        }
      }
    }
