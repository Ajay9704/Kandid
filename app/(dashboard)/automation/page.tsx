...
// API functions for automation settings
async function fetchAutomationSettings(): Promise<AutomationSettings[]> {
  // For now, we'll fetch from localStorage or return default settings
  ...
  try {
    ...
    console.warn('Failed to load automation settings from localStorage:', error)
  }
...
async function updateAutomationSettings(automation: AutomationSettings): Promise<AutomationSettings> {
  // Save to localStorage
  try {
    const settings = await fetchAutomationSettings()
    const updatedSettings = settings.map(s => s.id === automation.id ? automation : s)
    localStorage.setItem('automationSettings', JSON.stringify(updatedSettings))
    return automation
  } catch (error) {
...
async function toggleAutomation(id: string, enabled: boolean): Promise<{ id: string; enabled: boolean }> {
  try {
    const settings = await fetchAutomationSettings()
    const updatedSettings = settings.map(s => {
      if (s.id === id) {
        return { ...s, enabled, updatedAt: new Date() }
      }
      return s
    })
    localStorage.setItem('automationSettings', JSON.stringify(updatedSettings))
...
async function createAutomation(automation: Omit<AutomationSettings, 'id' | 'createdAt' | 'updatedAt' | 'userId'>): Promise<AutomationSettings> {
  try {
    const settings = await fetchAutomationSettings()
    ...
    const updatedSettings = [...settings, newAutomation]
    localStorage.setItem('automationSettings', JSON.stringify(updatedSettings))
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button onClick={() => {
            // Create a new automation with default settings
              ...
              templates: [
                {
                  id: '1',
                  step: 1,
                  content: "Hi {firstName}, I came across your profile and thought we should connect!",
                  delayDays: 0
                }
              ],
              filters: {
